<div class="row">
    <div class="col-lg-6 mb-3">
        ...
        <input type="text" class="form-control" id="event" required name="event"
            value="{{ isset($holidayDetail) ? $holidayDetail->event : old('event') }}" autocomplete="off"
            placeholder="">
    </div>
    ...
    <div class="col-lg-6 mb-3">
        ...
        <input class="form-control" name="event_date"
            ...
            @if (\App\Helpers\AppHelper::ifDateInBsEnabled()) type="text"
                  id="eventDate"
    ...
    <div class="col-lg-6 mb-3">
        ...
        <select class="form-select" id="branch-select" name="branch_id">
            ...
            @foreach($branches as $branch)
                <option value="{{ $branch->id }}" @if(isset($holidayDetail) && $holidayDetail->branch_id == $branch->id) selected @endif>{{ $branch->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="text-end">
        <button type="submit" class="btn btn-primary"><i class="link-icon"
                data-feather="{{ isset($holidayDetail) ? 'edit-2' : 'plus' }}"></i>
